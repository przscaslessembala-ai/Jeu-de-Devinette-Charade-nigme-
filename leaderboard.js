/*
	leaderboard.js — Classement en ligne via Firebase Firestore
	Auteur : P4
	Dépendances : Firebase SDK (CDN), leaderboard.html
*/
...
/* sélection des éléments du DOM via querySelector */
const lbLoading  = document.querySelector('#lb-loading');
...
const myBanner   = document.querySelector('#my-score-banner');
const myScoreVal = document.querySelector('#my-score-val');
const myRankVal  = document.querySelector('#my-rank-val');
const btnRefresh = document.querySelector('#btn-refresh');

/* chargement des scores depuis Firestore */
async function loadScores() {
	...
	try {
		...
		console.warn('[leaderboard] Impossible de charger depuis Firebase:', err.message);
		console.warn('[leaderboard] Utilisation du cache local comme fallback');
...
/* envoi du score vers Firestore (mise à jour si meilleur score) */
async function submitScore(entry) {
	if (!entry || !entry.pseudo || isNaN(entry.score)) {
		console.warn('[leaderboard] Entrée invalide, envoi annulé');
		return;
	}
	try {
		const q    = query(
		...
		console.warn('[leaderboard] Erreur lors de l\'envoi:', err.message);
		saveLocalScore(entry);
	}
}

/* sauvegarde locale en cas d'absence de connexion Firebase */
function saveLocalScore(entry) {
	const local = getLocalScores();
	const idx = local.findIndex(function(s) {
		return s.pseudo === entry.pseudo && s.level === entry.level && s.category === entry.category;
	});
	if (idx >= 0) {
...
/* filtrage selon période, niveau et catégorie */
...
function setupFilters(groupId, onSelect) {
	const groupe = document.querySelector('#' + groupId);
	if (!groupe) return;
	groupe.querySelectorAll('.filter-btn').forEach(function(btn) {
		btn.addEventListener('click', function() {
			groupe.querySelectorAll('.filter-btn').forEach(function(b) { b.classList.remove('active'); });
...
/* soumission automatique du score si on vient d'une partie */
